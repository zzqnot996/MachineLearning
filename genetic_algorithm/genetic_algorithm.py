...
class Population:
    ...
    def fitness_func(self, chrom1, chrom2):
        ...
        #------------------------------------------------------------------------------------------
        # 这段代码定义了三个函数，使用了Lambda表达式：
        # n(x, y): 这个函数计算了一个数学表达式的值，表达式为 sin(√(x² + y²))² - 0.5。
        # d(x, y): 这个函数计算了另一个数学表达式的值，表达式为 (1 + 0.001 * (x² + y²))²。
        # func(x, y): 这个函数使用了前两个函数的结果，计算了一个新的数学表达式的值，表达式为 0.5 - n(x, y)/d(x, y)。
        #--------------------------------------------------------------------------------------------
    ...
    def evolve(self):
        ...
        while True:
            ...
            #---------------------------------------------------------------------------------------------
            # 判断进化过程是否结束
            # 循环self.size/2次，每次从self.individuals 中选出2个
            #---------------------------------------------------------------------------------------------
            i = i + 2         
            if i >= self.size:
                break
    ...
    def run(self):
        ...
        for i in range (self.generation_max):
            self.evolve ()
            print ( "第", i ,"代", "个体适应度最大值: ", max (self.fitness), "个体适应度平均值: ", sum (self.fitness)/self.size, "个体适应度最小值: ", min (self.fitness))
